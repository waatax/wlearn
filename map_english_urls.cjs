// Map WLuznet English YouTube videos to books by S-code
const fs = require('fs');
const data = JSON.parse(fs.readFileSync('public/books.json', 'utf8'));

// Videos scraped from @WLuznet channel - mapped by S-code
const wluznetVideos = {
        // VW series
        ...
        // S4 series (from bottom of scroll)
...
// Also add VW-06C variant (same as VW-06)
wluznetVideos['VW-06C'] = wluznetVideos['VW-06'];
...
data.forEach(book => {
        ...
        // Try direct match
        let url = wluznetVideos[code];

        // Try without leading zeros (S1-08 -> S1-8)
        if (!url) {
                const m = code.match(/^(S\d+|VW)-0*(\d+C?)$/);
                if (m) {
                        url = wluznetVideos[`${m[1]}-${m[2]}`];
                }
        }
        ...
        if (url) {
                if (book.english_url) {
                        ...
                        updated++;
                        console.log(`Added: ${book.code} ${book.title_cn || book.title_en} -> ${url}`);
                }
        } else {
...
console.log(`\n--- Summary ---`);
console.log(`English URLs added: ${updated}`);
console.log(`Already had english_url: ${alreadyHad}`);
console.log(`Still missing (need more scraping): ${notFound}`);
console.log(`Total with english_url now:`, data.filter(b => b.english_url).length);
